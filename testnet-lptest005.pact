(module lptest005 GOVERNANCE
  ...
  (defschema sale
    @doc "Sale phase configuration"
    status:string        ; Sale status
    price:decimal       ; Current price
    start-time:time     ; Phase start
    end-time:time       ; Phase end
    max-per-account:integer ; Per-account limit
    total-available:integer ; Available supply
    total-sold:integer)    ; Total sold
  ...
  (deftable sale-status:{sale}
      @doc "Sale phase configuration and status")
...
;-----------------------------------------------------------------------------
  ...
  (defcap OWNER (account:string)
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
        (enforce (!= account "") "Account cannot be empty")
        (enforce-valid-account account)
        (enforce-guard (at "guard" (ref-coin::details account)))
    )
)
  ...
  (defun get-coin-ref:module{fungible-v2} ()
    (at "coin" (read-msg)))
...
(let ((ref-coin:module{fungible-v2} (get-coin-ref)))
    ...
    (enforce-guard (at "guard" (ref-coin::details sender)))
...
(defun nft-collection-request (
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      ...
      ;; Calculate fees
      (let* ((g:guard (at 'guard (ref-coin::details creator)))
          ...
          (cond
              ...
              ((contains creator discount_accounts)
                  [(with-capability (PRIVATE)
                      (install-capability (ref-coin::TRANSFER creator LAUNCHPAD_ACC discounted-fee))
                      (ref-coin::transfer creator LAUNCHPAD_ACC discounted-fee))])
              [(with-capability (PRIVATE)
                  (install-capability (ref-coin::TRANSFER creator LAUNCHPAD_ACC fee))
                  (ref-coin::transfer creator LAUNCHPAD_ACC fee))]
          )
...
(defun mint-for-free:bool
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      (let* (
          (account-guard:guard (at "guard" (ref-coin::details account)))
          ...
          ;; Burn priority pass with proper guard
          (with-capability (PRIVATE)
              (free.kmpasstest003.burn account account-guard))
          true
      ))
  )
          ...
          (cond
              ...
              (whitelist-enabled
                  [
                      ...
                      (enforce-whitelist account account-guard collection-name)
                      (let ((claimed:integer
                              (at 'claimed (get-whitelist-info account collection-name)))
                            (price:decimal (get-wl-price collection-name)))
                          
                          (with-capability (PRIVATE)
                              (install-capability (ref-coin::TRANSFER account creator (* amount price)))
                              (ref-coin::transfer account creator (* amount price)))
              ...
              [(check-public collection-name)
               (let ((mint-price:decimal (get-mint-price collection-name)))
                  (with-capability (PRIVATE)
                      (install-capability (ref-coin::TRANSFER account creator (* amount mint-price)))
                      (ref-coin::transfer account creator (* amount mint-price))))]
          )
      ))
  )
...
(defun add-whitelist
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      ...
      (let ((account-guard:guard (at 'guard (ref-coin::details account))))
          ;; Create whitelist entry with unique key
          (let ((whitelist-key:string (concat [collection-name "|" account])))
              (insert whitelists whitelist-key {
                  "account": account,
                  "guard": account-guard,
                  "claimed": 0,
              ...
              (let ((log-key:string (format "whitelist-{}-{}" 
                      [collection-name (hash (at "block-time" (chain-data)))])))
                  (write operation-logs log-key {
                      "operation": "add-whitelist",
                      "account": account,
                      "timestamp": (at "block-time" (chain-data)),
                      "details": {
                          "collection": collection-name,
...
(defun remove-whitelist-address
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      ...
      (let* (
          ...
          (account-guard:guard (at 'guard (ref-coin::details address)))
      )
          (if (= contain true)
              [(update collections collection-name {
              ...
              ;; Write with default values instead of delete
              ...
              ;; Log removal
              (let ((log-key:string (format "whitelist-remove-{}-{}" 
                      [collection-name (hash (at "block-time" (chain-data)))])))
                  (write operation-logs log-key {
                      "operation": "remove-whitelist",
                      "account": address,
                      "timestamp": (at "block-time" (chain-data)),
                      "details": {
                          "collection": collection-name,
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      (with-capability (RESERVE collection-name)
          ...
          (let (
              ...
              ;; Transfer payment with proper capability management
              (with-capability (PRIVATE)
                  (install-capability (ref-coin::TRANSFER account creator price))
                  (ref-coin::transfer account creator price))
      ...
      (let (
          ...
          (with-capability (MINT-NFT account)
              (let* (
                  ...
                  (acc-guard:guard (at "guard" (ref-coin::details account)))
                  ...
                  (let ((mint-result
                      ...
                      ;; Execute mint with proper coin handling
                      (with-capability (PRIVATE)
                          (mint-internal
                              collection-name
                              account
...
(defun airdrop-nft:string
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      ...
      (let* (
          ...
          ;; Transfer token with proper capability
          (with-capability (PRIVATE)
          ...
          ;; Record airdrop with unique key
          (let ((airdrop-key:string (format "airdrop-{}-{}" 
                  [collection-name (hash (at "block-time" (chain-data)))])))
              (insert airdrop-history-table airdrop-key {
                  'collection-name: collection-name,
                  'account: account,
                  'token-id: token_id,
          ...
          ;; Log airdrop operation
          (let ((log-key:string (format "airdrop-log-{}-{}" 
                  [collection-name (hash (at "block-time" (chain-data)))])))
              (write operation-logs log-key {
                  "operation": "airdrop-nft",
                  "account": account,
                  "timestamp": (at "block-time" (chain-data)),
                  "details": {
                      "collection": collection-name,
...
(defun add-priority-user
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      (with-capability (PRIVATE)
          ...
          ;; Get account guard using coin reference
          (let ((account-guard:guard (at 'guard (ref-coin::details account))))
              ...
              ;; Log operation with unique key generation
              (let ((log-key:string (format "{}-{}" 
                      ["add-priority" (hash (at "block-time" (chain-data)))])))
                  (write operation-logs log-key {
...
(defun revoke-priority
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      (with-capability (IS_ADMIN)
          ...
          ;; Get account guard using coin reference
          (let ((account-guard:guard (at 'guard (ref-coin::details account))))
              ;; Update priority status with revocation details
              ...
              ;; Log operation with unique key generation
              (let ((log-key:string (format "{}-{}" 
                      ["revoke-priority" (hash (at "block-time" (chain-data)))])))
                  (write operation-logs log-key {
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      (with-capability (IS_ADMIN)
          (let* (
              ...
              (creator-guard:guard (at 'guard (ref-coin::details creator)))
          )
              ;; Process refund with proper capability management
              (with-capability (PRIVATE)
                  (install-capability (ref-coin::TRANSFER LAUNCHPAD_ACC creator fee))
                  (ref-coin::transfer LAUNCHPAD_ACC creator fee))
  
              ;; Log denial with unique key generation
              (let ((log-key:string (format "{}-{}" 
                      ["deny-collection" (hash (at "block-time" (chain-data)))])))
                  (write operation-logs log-key {
              ...
              ;; Return success message
              (format "Collection {} denied and fee {} refunded to {}" 
                  [collection-name, fee, creator])
          )
      ))
  )
...
(defun sync-with-ng
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      ...
      (with-read minted-tokens (get-mint-token-id collection-name accountId)
          ...
          (let* (
              (collection-info:object{collection-details} (get-collection-details collection-name))
              (creator:string (at 'creator collection-info))
              (collection-id:string (at 'collection-id collection-info))
              (creator-guard:guard (at 'creator-guard collection-info))
              (account-guard:guard (at "guard" (ref-coin::details account)))
              (random:integer (get-random account))
              (account-minted:integer (get-account-minted account))
              (token-id:string (n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.ledger.create-token-id creator-guard uri))
              (token-precision 0)
              ...
              (policy:[module{n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.token-policy-ng-v1}] 
                  (n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.std-policies.to-policies policy_stack))
          )
              ;; Create and mint token with proper capabilities
              (with-capability (PRIVATE)
                  (n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.ledger.create-token 
                      token-id token-precision uri policy creator-guard)
                  (n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.ledger.mint 
                      token-id account account-guard 1.0))
  
              ;; Update token status
              (update minted-tokens (get-mint-token-id collection-name accountId)
                  {"revealed": true, "marmToken": token-id})
  
              ;; Record NFT info and update minted count
              ...
              ;; Log sync operation with unique key
              (let ((log-key:string (format "sync-{}-{}-{}" 
                      [collection-name, accountId, (hash (at "block-time" (chain-data)))])))
                  (write operation-logs log-key
...
(defun update-minted:string
    ...
    (let ((ref-coin:module{fungible-v2} (get-coin-ref)))
      ...
      (let* (
          (account-guard:guard (at 'guard (ref-coin::details account)))
          (current-time:time (at "block-time" (chain-data)))
          (new-minted:integer (+ minted 1))
      )
          ;; Update account details with proper guard
          (write account-details account {
              "account": account,
              "guard": account-guard,
              "minted": new-minted,
              "last-mint-time": current-time,
              "total-spent": 0.0,
              "collections": [],
              "roles": [],
          ...
          ;; Log the update with unique key
          (let ((log-key:string (format "minted-update-{}-{}" 
                  [account (hash current-time)])))
              (write operation-logs log-key {
          ...
          ;; Return success message
          (format "Successfully updated minted count for {} from {} to {}" 
              [account, minted, new-minted])
      ))
  )
